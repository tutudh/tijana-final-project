...
The oracle LP has a single global average-error constraint.
Let $\lambda\ge0$ denote the Lagrange multiplier, or shadow price, of this
constraint.
After eliminating the item-wise simplex multipliers, the dual reduces to the
one-dimensional concave problem
\begin{equation}
    \max_{\lambda\ge0}
    D(\lambda)
    :=
...
By finite-dimensional LP strong duality, the oracle LP and the reduced dual
\eqref{eq:dual-problem} have the same optimal value.
We use this duality only to motivate the fixed-price routing class; the
deployable method below selects and certifies such policies by calibration.
Appendix~\ref{app:dual-derivation} gives the explicit Lagrangian derivation.

\textbf{Roadmap.}
The discussion above is an oracle calculation.
If the losses $h_t(a)$ were known exactly, then the dual price would give the
right cost-aware routing rule: choose
$\lambda^\star\in\argmax_{\lambda\ge0}D(\lambda)$ in the one-dimensional dual,
and then route each item by minimizing
$C_t(a)+\lambda^\star h_t(a)$.
The obstacle is that $h_t(a)$ depends on the true label $Y_t$.
Thus the very quantity that makes the LP correct is unavailable before expert
labels are observed.

The rest of the paper studies two ways to make this oracle picture
deployable.
...
In the \textbf{online setting}, labels are revealed only intermittently through
random audits.
Whenever the true label is observed, we obtain feedback about the realized
loss $h_t$ and update the price $\lambda_t$ online through an
inverse-probability, gradient-descent-like dual update.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{results/two_source_offline_pac/two_source_offline_pac_summary.png}
