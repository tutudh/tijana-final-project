...
the dual minimization separates across items, and the action for item $t$ is given by the following rule:
\begin{equation}
    a_t(\lambda)
    \in
    \argmin_{a\in\calA}
...
The labeling losses $h_t(j)$ are unknown if expert labels are
not collected.
...
For example, replacing $h_t$ by $\hath_t$ in the risk constraint of
\eqref{eq:oracle-lp} would produce a proxy-constrained LP, and the rule
...
the fixed-price rule of Section~\ref{sec:calibration-selection} applied to
the constructed two-source pool; it uses the finite-class Hoeffding union
bound \eqref{eq:hoeffding-ucb} over the same $\lvert\Lambda\rvert=202$
threshold grid as the simple offline experiment, because the multi-source
routed loss is no longer monotone in a scalar threshold. \emph{Source-$j$
monotone PAC} ($j\in\{A,B\}$) is the original PAC baseline restricted to one
source. \emph{Best-of-singles (Bonferroni)} runs both single-source baselines
at level $\alpha/2$ and picks the source with the larger empirical savings on
the calibration sample, which remains a valid $\alpha$-PAC procedure.
...
in the appendix reports the full sweep). A tighter
deviation bound (Bernstein, or a $\sup$-deviation argument exploiting the
unidimensional sufficient statistic $\min_j\hath_{t,j}$) would close this gap;
