...
\zwy{How to solve this problem? and here is a constraint on the size of valibration set.}


\subsection{Offline Labeling with Proxy Losses}
...
\begin{equation}
    ...
    \label{eq:C-lambda}
\end{equation}
Writing $a^\lambda_{1:T}=(a_1^\lambda,\ldots,a_T^\lambda)$ for the proxy-price
policy, these are exactly the per-item loss and cost of
\eqref{eq:itemwise-loss-cost} evaluated at $a_t^\lambda$, namely
$H_t(\lambda)=h_t(a_t^\lambda)$ and $C_t(\lambda)=c_t(a_t^\lambda)$.
The finite-pool averages are therefore the functionals $L_T,C_T$ of
\eqref{eq:finite-pool-risk-cost} at this policy,
\begin{equation}
    L_T(\lambda):=L_T(a^\lambda_{1:T})=\frac1T\sum_{t=1}^T H_t(\lambda),
    \qquad
    C_T(\lambda):=C_T(a^\lambda_{1:T})=\frac1T\sum_{t=1}^T C_t(\lambda).
    \label{eq:L-C-lambda}
\end{equation}

Recall from Section~\ref{sec:optimization-dual} that cheap sources are free
and the expert costs 1. Then the implementable rule
\eqref{eq:true-price-action} sends item $t$ to
$j_t^\lambda=\argmin_{j}(c_{t,j}+\lambda\hath_{t,j})$ unless deferral is cheaper.
Define the \emph{best-source} proxy uncertainty
\begin{equation}
    \hath_t^\star := \min_{j\in[k]} \hath_{t,j},
    \qquad
    j_t^\star \in \argmin_{j\in[k]} \hath_{t,j},
    \label{eq:best-source-score}
\end{equation}
with $j_t^\star$ broken by a fixed, price-independent rule. Because all cheap
costs are equal, $j_t^\lambda=j_t^\star$ for every $\lambda>0$: the price never
reselects the source. The deferral rule \eqref{eq:proxy-defer} then collapses
to a single scalar threshold on the score $\hath_t^\star$,
\begin{equation}
    D_t^\lambda = \ind{\hath_t^\star \ge 1/\lambda},
    \label{eq:scalar-threshold}
\end{equation}
so item $t$ is routed to its fixed best source iff $\hath_t^\star<1/\lambda$.
Thus $\hath_t^\star$ plays exactly the role of the uncertainty score in
single-source PAC labeling, with induced threshold $\tau(\lambda)=1/\lambda$;
raising $\lambda$ only moves items from route to defer. A deferred item incurs
zero loss and a routed item incurs the fixed loss
$\ell(Y_t,\hatY_t^{(j_t^\star)})$, so the per-item certificate increment
$H_t(\lambda)$ of Section~\ref{sec:proxy-estimation}, and hence $L_T(\lambda)$
and $\hatL_m(\lambda)$, is non-increasing in $\lambda$. The induced policies
form the nested family $\{\Pi_\lambda\}_{\lambda\ge0}$ with at most $T+m$
distinct breakpoints $\{1/\hath_t^\star\}$; the candidate set $\Lambda$ is this
breakpoint set together with the expert-only policy ($\lambda=\infty$,
$D_t^\infty=1$, $L_T(\infty)=0$). It may depend on the target covariates,
cheap-source predictions, and any separate training data used to learn $\hath$,
but it must not depend on the calibration labels used below.



% \begin{remark}[Unequal cheap-source costs]
% \label{rmk:unequal-costs}
% The reduction uses equal cheap-source costs (here $c_{t,j}\equiv0$). With
% unequal costs the minimizer $j_t^\lambda$ can switch sources as $\lambda$
% varies, at prices $\lambda=(c_{t,b}-c_{t,a})/(\hath_{t,a}-\hath_{t,b})$, and a
% switch can raise a routed item's true loss, so $L_T(\lambda)$ need not be
% monotone. One then reverts to a finite candidate set $\Lambda$ and the uniform
% Hoeffding bound
% \begin{equation}
%     U_m^{\mathrm{unif}}(\lambda;\alpha)
%     =
%     \hatL_m(\lambda)+\sqrt{\frac{\log(|\Lambda|/\alpha)}{2m}},
%     \label{eq:hoeffding-ucb-uniform}
% \end{equation}
% valid over all of $\Lambda$ by a union bound;
% Appendix~\ref{app:not-very-meaningful} bounds the induced policy count.
% \end{remark}
...
\paragraph{Relation to the single-model PAC labeling}
...
The quantities $j_t^\star$, $D_t$, and the loss $H_t$ defined below are the
online counterparts of the offline $j_t^\lambda$, $D_t^\lambda$, and
$H_t(\lambda)$ from Section~\ref{sec:proxy-estimation}, evaluated at the
time-varying price $\lambda_t$; we drop the explicit price superscript since it
is implied by the round index $t$.
...
\]
where deferral has zero loss because the expert label is used.
Deferral is an observed zero-loss event, while an unaudited AI decision has
unobserved loss.  Define the augmented observation mask and its conditional
observation probability by
\begin{equation}
    O_t=D_t+(1-D_t)A_t,
    \qquad
    q_t=D_t+(1-D_t)p_t .
...
\end{equation}
Thus $q_t=1$ when $D_t=1$ and $q_t=p_t$ when $D_t=0$.
The centered inverse-observation update signal is
\begin{equation}
    \widehat Z_t
    =
...
$(X_t,\{\hatY_t^{(j)},\hath_{t,j},c_{t,j}\}_{j=1}^k,c_{t,E})$.
...
\]
If future costs are not known in advance, these extrema may be replaced by any
deterministic lower bound on cheap-source costs and upper bound on expert
costs that are known before deployment.
\end{assumption}
