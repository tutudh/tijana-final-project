...
\author[Zhang, Hao, Tu]{Wanyu Zhang \and Lutong Hao \and Daohong Tu}
\institute[]{STATS 357 --- Final Project}
\date{}
...
% ----------------------------------------------------------- MOTIVATION
\begin{frame}{The problem: labeling a big pool, cheaply}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      We have a pool of unlabeled items $X_1,\dots,X_T$. We must release a label
      for each one.
      \medskip

      Two ways to get a label:
      \begin{itemize}
        \item \textbf{Cheap ML models} --- free, but sometimes wrong.
        \item \textbf{Expert} --- always correct, but \emph{expensive}.
      \end{itemize}
      \medskip

      \textbf{Goal.} Use the cheap models as much as possible, while
    ...
    \begin{column}{0.44\textwidth}
      \begin{block}{The tension}
        \centering
        more AI $\;\Rightarrow\;$ lower cost\\[2pt]
        more expert $\;\Rightarrow\;$ lower error
      \end{block}
      \begin{block}{Our angle}
        Treat it as a \textbf{linear program}, and read everything off
        the LP \emph{dual}.
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Setup and notation}
  \begin{itemize}
    \item Action set $\calA=\{E,1,\dots,k\}$: defer to the \textbf{expert} $E$,
          or accept one of $k$ \textbf{cheap models}.
    \item For item $t$ and action $a$, a \textbf{loss} and a \textbf{cost}:
  \end{itemize}
  \[
     h_t(a)=
...
% --------------------------------------------------------- SETUP DIAGRAM
\begin{frame}{Setup schematic: one item, many actions}
  \centering
...
\begin{frame}{The cost-aware labeling LP}
  ...
  \begin{itemize}
    ...
    \item If true losses $h_t(a)$ were known, this LP \emph{is} the optimal policy.
  \end{itemize}
\end{frame}
